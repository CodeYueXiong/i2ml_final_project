...
param_k_kernel_dist <- ParamSet$new(params = list(ParamInt$new("k", lower = 40, upper = 100),
                                             ParamFct$new("kernel", levels = kernel_type),
                                             ParamInt$new("distance", lower = 1, upper = 3)))
...
knn_auto_kern <- AutoTuner$new(learner = knn_learner, resampling = inner_rsmp, 
                           measures = measure, tune_ps = param_k_kernel,
                           terminator = term("none"), tuner = tnr("grid_search", resolution = 10))

knn_auto_kern_dist <- AutoTuner$new(learner = knn_learner, resampling = inner_rsmp, 
                           measures = measure, tune_ps = param_k_kernel_dist,
                           terminator = term("none"), tuner = tnr("grid_search", resolution = 10))
...
design_kern_dist <- benchmark_grid(
  tasks = task,
  learners = knn_auto_kern_dist,
  resampling = outer_rsmp
)
...
bmr_kern <- run_benchmark(design_kern) # 424.65 sec elapsed
bmr_kern_dist <- run_benchmark(design_kern_dist)
...
k_dist_kern_path = bmr_kern_dist$data$learner[[1]]$archive("params")
knn_ggp4 = ggplot(k_dist_kern_path, aes(
  x = k,
...
The kernel function is a additional weighted distance....

# based on all info

```{r}
knn_learn_all <- lrn("classif.kknn", predict_type = "prob", distance=1, kernel = "inv")
...
# setting the tunning for parameters, and terminator
knn_param_set <- ParamSet$new(params = list(ParamInt$new("classif.kknn.k", lower = 25, upper = 65),
