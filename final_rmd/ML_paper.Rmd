...
```{r figs, echo=FALSE,fig.cap="\\label{fig:figs} Flowchart: process of experiment", fig.pos="h"}
...
The bart chart below illustrates how the variable is discretizated into categories.
As for variable `CNT_CHILDREN` we notice that there are 3 categories, i.e. `[-inf, 1)`,`[1,2)`,`[2, inf)`(shown as horizontal axis). The color "`bad`" and "`good`" represent the sample with target variable(y) value "1" and "0". The height of each bar represents the sample proportion, whose target value is "`bad`"(1), to the whole sample (shown as right vertical axis "Bad probability") and the sample proportion of each category to the whole sample (shown as left vertical axis "Bin count distribution"). The numbers written at the top of each bar show the percentage of each category. The most important ratios, bad ratios, representing the proportion of "`bad`" samples to "`good`" samples in each category, are shown inside of the bar. We notice the bad ratios for categories are linear, which means this binning is good.
...
`[-inf, -19000]`,`[-19000, -175000]` etc.  and the bad ratios for categories are linear, which means 
...
As for **smote**, we set hyperparamters *smote.K* ranging from 20 to 60 and  *smote.dup_size* ranging from 10 to 20.
...
```{r echo=FALSE,out.width = "100%", fig.pos="h"}
knitr::include_graphics("img/smote_lg.png")
```

shown on the plots above, it is better to choose *smote.K* with value 
40 and *smote.dup_size* with value 15 in order to show a considerably good prediction performance measured around **0.72**.  
...
As we can see in the plot, we tried using **SMOTE** with different combinations of *smote.K* and *smote.dup_size*. With three-fold cross-validation, we see three plots, and in two out of three plots, the pink curve (*smote.K*=20) performs the best, specifically with *smote.dup_size* being 40. We will fix these values after further fine-tune the KNN model.
...
The first plot shows that the AUC grows as we increase the *distance* parameter in KNN, but after 3, there is no significant increase. We fix distance to 3 for further tuning. Based on the fixed distance, we compare results from different kernels in the second plot. Here, there is no clear winner, so we simply take the best from each cross-validation step, being *rectangular*, *triangular* and *gaussian*. And in the next plot, we can directly eliminate *rectangular*, as it has the lowest AUC in also cases. *triangular* and *gaussian* have very similar performance in terms of AUC and *gaussian* is slightly better in most cases.
...
```{r eval=FALSE}
library(ranger) # load the required package "Ranger" for Random Forest
## tune for best matches with oversampling's parameter

# set learner with fixed number of trees: 1000 as suggested by algorithm "tuneRanger"
rf_lrn <- lrn("classif.ranger", predict_type = "prob", num.trees = 1000)
# train with oversampling function's parameter
...
# create oversampling learner fixed in rf_trees_lrn
rf_over_lrn <- GraphLearner$new(po_over %>>% rf_lrn, predict_type = "prob")

# single parameter to train with oversampling，sample size
param_over = ParamSet$new(list(ParamDbl$new("oversample.ratio", lower = 30, upper = 60)))

# create Autolearner
rf_auto_over <- AutoTuner$new(
  learner = rf_over_lrn, resampling = inner_rsmp,
  measures = msr("classif.auc"), tune_ps = param_over,
  terminator = term("none"), tuner = tnr("grid_search", resolution = 30)
)
# outer resampling set
outer_rsmp <- rsmp("cv", folds = 3L)
# design for oversampling
design_over <- benchmark_grid(
  tasks = task,
  learners = rf_auto_over,
  resampling = outer_rsmp
)
# set seed
set.seed(2020)
# run benchmark
bmr_over <- benchmark(design_over, store_models = TRUE)
# plot path, there will be 3 plots in all cuz the existence of a 3-fold CV
over_path1 <- bmr_over$data$learner[[1]]$archive("params")
rf_ggp1 <- ggplot(over_path1, aes(
  x = oversample.ratio,
  y = classif.auc
)) +
  geom_point(size = 3) +
  geom_line() #+ theme(legend.position = "none")
over_path2 <- bmr_over$data$learner[[2]]$archive("params")
rf_ggp2 <- ggplot(over_path2, aes(
  x = oversample.ratio,
  y = classif.auc
)) +
  geom_point(size = 3) +
  geom_line() #+ theme(legend.position = "none")
over_path3 <- bmr_over$data$learner[[3]]$archive("params")
rf_ggp3 <- ggplot(over_path3, aes(
  x = oversample.ratio,
  y = classif.auc
)) +
  geom_point(size = 3) +
  geom_line() #+ theme(legend.position = "none")
# load the required package for plotting the performance of comparison pics
library(ggpubr)
ggarrange(rf_ggp1, rf_ggp2, rf_ggp3, common.legend = TRUE, legend = "bottom")
```
```{r echo=FALSE,out.width = "100%", fig.pos="h"}
